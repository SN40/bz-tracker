...
def upload_csv():
    ...
    try:
        ...
        # Interne CSV-Duplikate bereinigen
    ...
    try:
        if should_delete:
            ...
            # Bestehende Daten in ein Set laden für schnellen Abgleich
            existing_dates = {m.date_mess for m in Mess.query.filter_by(user_id=current_user.id).all()}
        
        # WICHTIG: Diese Schleife darf NICHT im "if should_delete" Block eingerückt sein
        for row in reader:
            csv_date = row.get('Datum')
            if hasattr(csv_date, 'to_pydatetime'):
                csv_date = csv_date.to_pydatetime()

            # Wert- und Notiz-Verarbeitung (jetzt sicher außerhalb von Bedingungen)
            try:
                wert_raw = row.get('Wert')
                wert = int(float(wert_raw)) if pd.notna(wert_raw) and wert_raw != '' else 0
            except (ValueError, TypeError):
                wert = 0

            raw_notiz = str(row.get('Notiz', ''))
            notiz = "" if raw_notiz.lower() == 'nan' or not raw_notiz else raw_notiz[:200]

            # Kollisions-Check: Wenn Zeitstempel belegt, Minute hochzählen
            while csv_date in existing_dates:
                csv_date += timedelta(minutes=1)
                # uebersprungen += 1 # Falls du lieber zählen willst, wie oft verschoben wurde
            ...
            existing_dates.add(csv_date)
            neu += 1

        db.session.commit()
        flash(f"Erfolg! {neu} Datensätze verarbeitet.", "success")
